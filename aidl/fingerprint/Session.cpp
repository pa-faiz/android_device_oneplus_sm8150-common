...
#undef LOG_TAG
#define LOG_TAG "OneplusUdfpsHalSession"
...
Session::Session(fingerprint_device_t* device, int32_t sensorId, int32_t userId,
            std::shared_ptr<ISessionCallback> cb, LockoutTracker lockoutTracker)
            : mDevice(device), mSensorId(sensorId), mUserId(userId), mCb(cb), mLockoutTracker(lockoutTracker) {
